...
### 1주차 (3/4) ⚠️ 수요일 1회 수업 (3/2 삼일절 대체공휴일)
...
- (강의) 조건문 (if/elif/else), 반복문 (for, while)
- (강의) 리스트 기초
- (실습) 교통사고 위치명 리스트에서 '서울' 포함 항목 필터링
- (실습) 사고건수 리스트 합계, 평균, 최댓값 계산
- (실습) for문으로 사고유형별 건수 세기
...
- (강의) 파일 입출력 (txt, csv 읽기/쓰기)
- (강의) 외부 라이브러리 설치 (pip install)
- (실습) 교통사고 XLS → CSV 변환 후 파이썬으로 읽기
- (실습) 딕셔너리로 시도별 사고건수 집계 함수 작성
- (과제) "2024년 보행노인 사고" 필터링 → 시도별 건수 → CSV 저장
...
- (강의) 필터링, 정렬, 그룹화 (groupby), 집계, 피벗 테이블
- (강의) **Pandas → JSON 변환** (이후 JS 시각화 연동의 핵심)
- (실습) 교통사고 데이터 본격 분석: 사고유형별, 연도별, 시도별 집계
- (실습) JSON 변환: `accidents_by_region.json`, `accidents_by_year.json` 저장
- (과제) 조별 프로젝트 주제 후보 3개 선정 + 관련 데이터 소스 탐색 보고서
...
### 13주차 (5/27) ⚠️ 수요일 1회 수업 (5/25 부처님오신날 대체공휴일)
...
### 14주차 (6/1) ⚠️ 월요일 1회 수업 (6/3 지방선거 공휴일)
